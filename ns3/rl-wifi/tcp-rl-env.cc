...
{
  NS_LOG_UNCOND ("TCPRLENV::RXPKTTRACE");
...
{
  NS_LOG_UNCOND ("TcpTimeStepEnv::ScheduleNextStateRead");
  ...
  std::cerr << "Time: " << (uint64_t)(Simulator::Now().GetMilliSeconds()) << " env->ssThresh: " << env->ssThresh << " env->cWnd: "
            << env->cWnd << " env->segmentSize: " << env->segmentSize << " bytesInFlightSum: " << bytesInFlightSum << std::endl;
  SetCompleted();
  ...
  //   env->event = m_event;
  //   env->ecnState = m_tcb->m_ecnState;
  std::cerr << "Time: " << Simulator::Now().GetMilliSeconds() << " new_cWnd: " << m_new_cWnd << " new_ssThresh: " << m_new_ssThresh
            << std::endl;
}
