...
{
  NS_LOG_UNCOND ("TcpRlTimeBased::TcpRlTimeBased:TcpCongestionOps");
  m_tcpSocket = 0;
}

TcpRlTimeBased::TcpRlTimeBased (const TcpRlTimeBased &sock) : TcpCongestionOps (sock)
{
  NS_LOG_UNCOND ("TcpRlTimed::TcpRlTimeBased:TcpCongestionOps(sock)");
  m_tcpSocket = 0;
}
...
TcpRlTimeBased::ConnectSocketCallbacks ()
{
      ...
      for (uint32_t j = 0; j < sockNum; j++)
        {
          ...
          NS_LOG_UNCOND ("Node: " << node->GetId () << " TCP Socket: " << tcpSocket);
          if (!tcpSocket)
            {
              continue;
            }
          ...
          NS_LOG_UNCOND ("CA name: " << ca->GetName ());
  ...
  if (m_tcpSocket)
    {
      NS_LOG_UNCOND ("Found TCP Socket: " << m_tcpSocket);
...
{
  NS_LOG_UNCOND ("TcpRlTimeBased::GetSsThresh " << state << bytesInFlight);
  if (!m_cbConnect)
    {
      m_cbConnect = true;
      CreateEnv ();
...
{
  NS_LOG_UNCOND ("TcpRlTimeBased::IncreaseWindow " << tcb << segmentsAcked);
  if (!m_cbConnect)
    {
      m_cbConnect = true;
      CreateEnv ();
...
{
  NS_LOG_UNCOND ("TcpRlTimeBased::PktsAcked");
  if (!m_cbConnect)
    {
      m_cbConnect = true;
      CreateEnv ();
...
{
  NS_LOG_UNCOND ("TcpRlTimeBased::CongestionStateSet");
  if (!m_cbConnect)
    {
      m_cbConnect = true;
      CreateEnv ();
...
{
  NS_LOG_UNCOND ("TcpRlTimeBased:CwndEvent");
  if (!m_cbConnect)
    {
      m_cbConnect = true;
      CreateEnv ();
