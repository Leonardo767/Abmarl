...
class CommunicationWrapper(Wrapper):
    ...
    in the environment. When an agent chooses to send, recipient agents will see in their next
    ...
    to the fuse_observation function, which must be implemented by the wrapped environment.

    Agents' observation and action spaces are converted into dictionaries. The
    action and observation from the environment are keyed on 'env_action' and 'env_obs',
    ...
    def __init__(self, env):
        ...
        for agent in self.agents.values():
            action_space_helper = {'env_action': agent.action_space}
            ...
            obs_space_helper = {'env_obs': agent.observation_space}
    ...
    def get_obs(self, agent_id, **kwargs):
        ...
        """
        obs_from_env = self.env.get_obs(agent_id, fusion_matrix=self.received_message[agent_id])
        return {'env_obs': obs_from_env, 'message_buffer': self.message_buffer[agent_id]}
